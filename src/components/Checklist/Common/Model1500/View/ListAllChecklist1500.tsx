...
const ListAllChecklist1500 = () => {
  ...
  const onDeleteHandle = (ev: number) => {
    ...
    axios
      .delete(
        "http://" + process.env.REACT_APP_URL + "/api/checklist1500/" + ev
      )
      ...
      <IonList>
        {checklists.map((result, index) => (
          <IonItem key={index}>
            ...
            <IonButton routerLink={`${"/checklist/1500/edit/" + result._id}`}>
              Edit
            </IonButton>
            <IonButton routerLink={`${"/checklist/1500/view/" + result._id}`}>
              View
            </IonButton>
...
export default ListAllChecklist1500;
